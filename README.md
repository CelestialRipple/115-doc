...
MoviePilot V2/V3 插件“腾讯文档115媒体库”。插件可将多个腾讯文档中的 115 分享资源同步到本地目录，使用 MoviePilot 识别和刮削并生成 STRM；播放或下载时按需获取 115 临时直链，不转存到个人网盘。支持可配置并行刮削、跨工作表复用元数据、按工作表分开目录、现有目录免刮削迁移，以及后台任务暂停与恢复。
